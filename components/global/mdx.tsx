...
export default function MDX({ source }: { source: MDXRemoteProps }) {
  ...
  return (
    <article
      className={`prose-md prose-stone prose-a:text-stone-900 hover:prose-a:text-stone-900" sm:prose-lg dark:prose-invert text-left ${styles.root}`}
...
function ExamplesCard({ data }: { data: ExampleCardProps }) {
  return (
    <a href={`https://${data.url}`} target="_blank" rel="noreferrer">
      <div className="ease hidden rounded-2xl border-2 border-stone-100 bg-white shadow-md transition-all duration-200 hover:-translate-y-1 hover:shadow-xl lg:block">
        ...
        <div className="h-36 px-5 py-6">
          <h3 className="truncate font-title text-2xl font-bold tracking-wide">
            {data.name}
          </h3>
          <p className="mt-3 text-base italic leading-snug text-stone-800">
            {data.description}
          </p>
        </div>
      </div>
      <div className="ease flex h-36 items-center overflow-hidden rounded-xl border-2 border-stone-100 bg-white transition-all duration-200 focus:border-black active:border-black md:h-48 lg:hidden">
        ...
        <div className="w-3/5 px-5 py-6">
          ...
          <p className="mt-3 text-sm font-normal italic leading-snug text-stone-800">
            {data.description}
          </p>
        </div>
      </div>
