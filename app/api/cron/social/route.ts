...
export async function GET(request: NextRequest) {
  ...
  console.log("Iniciando a criação do post para redes sociais");
    ...
    try {
      ...
      const completion = await openai.chat.completions.create({
        messages: [
          ...
          O post deve incluir palavras-chaves para melhorar o SEO do instagram. 
      ...
      const SOCIAL_COLLECTION = `${queue.siteId}`;
      try {
        await clientTypesense.collections(SOCIAL_COLLECTION).retrieve();
      } catch (error) {
        await clientTypesense
          .collections()
          .create({ ...socialSchema, name: SOCIAL_COLLECTION });
      }

      await clientTypesense
        .collections(SOCIAL_COLLECTION)
        .documents()
        .upsert({ ...social, type: "instagram" });

      await prisma.queue.create({
        data: {
          description: "social_instagram",
