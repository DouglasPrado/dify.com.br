...
import { resendProductAI } from "@/lib/actions/product";
...
export default function ResendProductAIButton({
  productId,
}: {
  productId: string;
}) {
    ...
    <button
      onClick={() =>
        startTransition(async () => {
          await resendProductAI(productId);
          va.track("Resend Product AI");
          toast.success("Aguarde a execução da IA!");
          router.push(`/site/${id}/sales/products?updated=${productId}`);
        })
      }
      className={cn(
