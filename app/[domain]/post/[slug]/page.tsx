            ...
            <div className="flex w-full flex-col gap-3">
              {collections.map(
                (
                  ...
                  <Link
                    className="hover:-transform-y-1 flex w-full items-center justify-between gap-2  rounded p-4 text-sm text-stone-700 transition-all duration-200 hover:bg-stone-100"
                    href={`/`}
      ...
      {data.adjacentPosts && (
        <div className="mx-5 mb-20 grid max-w-7xl grid-cols-1 gap-x-4 gap-y-8 md:grid-cols-2 xl:mx-auto xl:grid-cols-3">
          {data.adjacentPosts.map((data: any, index: number) => (
            <BlogCard key={`key-post-adjacent-posts-${index}`} data={data} />
          ))}
        </div>
      )}
      <FooterSection
